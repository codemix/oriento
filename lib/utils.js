...
exports.extend = function(source) {
  ...
  var keys, key, g, limit;

  for(keys = Object.keys(parent), key = null, g = 0, limit = keys.length; g<limit; g++) {
    key = keys[g];
  ...
  for(keys = Object.keys(source), key = null, g = 0, limit = keys.length; g<limit; g++) {
    key = keys[g];
...
exports.prepare = function(query, params) {
  ...
  var pattern = /"(\\[\s\S]|[^"])*"|'(\\[\s\S]|[^'])*'|:([A-Za-z_-]+|\/\*[\s\S]*?\*\/)/;
