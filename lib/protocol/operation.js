...
Operation.prototype.writeLong = function(data) {
  this.writeOps.push([constants.BYTES_LONG, function(buffer, offset) {
    ...
    buffer.fill(0, offset, constants.BYTES_LONG);
...
Operation.prototype.writeString = function(data) {
  ...
  this.writeOps.push([constants.BYTES_INT + length, function(buffer, offset) {
    buffer.fill(0, offset, offset + constants.BYTES_INT + length);
    buffer.writeInt32BE(length, offset);
    buffer.write(data, offset + constants.BYTES_INT);
  }]);
...
Operation.prototype.readStatus = function(fieldName, reader) {
  ...
  self.readByte('code');
  self.readInt('sessionId', function(data) {
    if(data.code === 1) {
      self.readError('error', function() {
        if(reader) {
          reader.call(self, value, fieldName);
        }
        self.readOps.push(function() {
          self.stack.pop();
        });
      });
    } else {
...
Operation.prototype.readObject = function(fieldName, reader) {
  this.readOps.push(['String', [fieldName, function(data, fieldName) {
    data[fieldName] = deserializer.deserializeValue('{' + data[fieldName] + '}');
    if(reader) {
...
Operation.prototype.consume = function(buffer, offset) {
  ...
  while(this.readOps.length) {
    item = this.readOps.shift();
...
/**
 * Determine whether the operation can read a value of the given
...
Operation.prototype.parseRecord = function(buffer, offset, context, fieldName, reader) {
  ...
  self.readShort('classId', function(record, fieldName) {
    if(record[fieldName] === -1) {
      record.value = new errors.Protocol('No class for record, cannot proceed.');
      self.stack.pop();
      self.readOps.push(function() {
        if(reader) {
          reader.call(self, context, fieldName);
        }
      });
      self.readOps.push.apply(self.readOps, remainingOps);
      return;
    }
      ...
      self
        ...
        .readString('value', function(data, key) {
          data[key] = deserializer.deserializeDocument(data[key]);
          self.stack.pop();
          self.readOps.push(function() {
            if(reader) {
              reader.call(self, context, fieldName);
...
Operation.prototype.parseArray = function(buffer, offset, context, fieldName, reader) {
  var self = this,
    items = reader.call(self, context),
    remainingOps = self.readOps;

  self.readOps = [];

  context[fieldName] = [];
  self.stack.push(context[fieldName]);

  items.map(function(item) {
    var childContext = {};
    context[fieldName].push(childContext);
    self.readOps.push(function() {
      self.stack.push(childContext);
    });

    item.call(self, childContext);
  ...
  self.readOps.push.apply(this.readOps, remainingOps);
  return 0;
};
...
Operation.prototype.parseError = function(buffer, offset, context, fieldName, reader) {
  ...
  function readItem() {
    self.readString('type');
    self.readString('message');
    self.readByte('hasMore', function(data) {
      if(data.hasMore) {
        var prev = new errors.Request();
        ...
        self.readBytes('javaStackTrace', function() {
          self.readOps.push(function() {
            self.stack.pop();
          });
        });
...
Operation.prototype.parsePushedData = function(buffer, offset, context, fieldName, reader) {
  ...
  switch(asString.charAt(0)) {
    case 'R':
      context[fieldName] = deserializer.deserializeValue('{' + asString.slice(1) + '}');
      break;
    default:
