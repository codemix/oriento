...
Bag.prototype._parse = function () {
  ...
  if (this._type === Bag.BAG_EMBEDDED) {
    this._size = buffer.readUInt32BE(this._offset);
    this._offset += 4;
  }
  else {
    ...
    this._offset += 8;
    this._pageOffset = buffer.readUInt32BE(this._offset);
    this._offset += 4;
    this._size = buffer.readUInt32BE(this._offset);
    this._offset += 4;
    this._changeSize = buffer.readUInt32BE(this._offset);
    this._offset += 4;
  }
  this._buffer = buffer;
};
...
Bag.prototype.next = function () {
  ...
  if (this._type === Bag.BAG_EMBEDDED) {
    ...
    if (this.resolvableRecords && this.resolvableRecords[rid]) {
      return this.resolvableRecords[rid];
    }
    else {
      return rid;
    }
