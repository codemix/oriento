...
function RecordID (input) {
  ...
  if (input) {
    if (typeof input === 'string') {
      ...
      if (input.cluster == +input.cluster) this.cluster = +input.cluster;
      if (input.position == +input.position) this.position = +input.position;
    }
  }
}
 ...
 *
 * @param  {String|Array|Object}          input The input to parse.
 * @return {RecordID|RecordID[]|Boolean}        The parsed RecordID instance(s) or false if the record id could not be parsed
 */
