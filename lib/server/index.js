...
Server.prototype.applySettings = function (options) {
  ...
  this.password = options.password || '';
	...
	//Return RID as an object ('o') or string ('s')
	this.ridFormat = options.ridFormat ? options.ridFormat.toLowerCase() : 'o';
...
Server.prototype.bindToSocket = function () {
  ...
  this.ops.on('error', function (err) {
    this.logger.debug('Operation error received, closing socket.');
    this.socket.end();
    this.ops.cancel(err);
    this.unbindFromSocket();
    this.emit('error', err);
