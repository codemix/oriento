...
function deserializeValue (value) {
  ...
  if (firstChar === '"') {
    ...
    // date
    return new Date(parseInt(value.substring(0, value.length)));
  }
  else if (firstChar === '(') {
    // object / document
    return deserializeDocument(value.substring(1, value.length - 1));
  }
  if (firstChar === '{') {
    // map
    return deserializeDocument(value.substring(1, value.length - 1), {}, true);
  }
  else if (firstChar === '[' || firstChar === '<') {
    //process Set <...> like List [...]
    values = splitList(value.substring(1, value.length - 1));
    ...
    // byte
    return String.fromCharCode(+(value.substring(0, value.length)));
  }
  else if (lastChar === 'l' || lastChar === 's' || lastChar === 'c') {
    // integer
    return parseInt(value.substring(0, value.length), 10);
  }
  else if (lastChar === 'f' || lastChar === 'd') {
    // float / decimal
    return parseFloat(value.substring(0, value.length));
  }
  else if (+value == value) {
    // integer
    return +value;
