...
CLI.prototype.run = function(argv) {
  ...
  return this.parseArgv(argv)
    .then(function(argv) {
      ...
      command = self.createCommand(command, argv);
      total = command.requiredArgv.length;

      for(var g = 0; g<total; g++) {
        if(typeof argv[command.requiredArgv[g]] === 'undefined') {
          yargs.demand(command.requiredArgv[g]);
          return command.help();
        }
      }
...
CLI.prototype.parseArgv = function(argv) {
  ...
  return self.declareArgv()
    .then(function() {
      ...
      return self.parseOptsFile(path.join(interim.cwd, 'oriento.opts'))
        .then(function(opts) {
...
CLI.prototype.declareArgv = function() {
  return this.list()
    .then(function(names) {
      return yargs
        .usage('Usage: oriento [OPTIONS] [' + names.join('|') + ']')
        .options({
          d: {
            alias: 'cwd',
            default: process.cwd(),
