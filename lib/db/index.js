...
Db.prototype.configure = function (config) {
  ...
  this.type = config.type === 'document'
              ? 'document'
              : 'graph';
...
Db.prototype.query = function (command, options) {
	options = options || {};
	options.class = options.class || 'com.orientechnologies.orient.core.sql.query.OSQLSynchQuery';
...
Db.prototype.normalizeResult = function (result) {
  ...
  if (Array.isArray(result)) return result.map(this.normalizeResult, this);
...
Db.prototype.normalizeResultContent = function (content) {
  ...
  if (Array.isArray(content)) return content.map(this.normalizeResultContent, this);
  ...
  if (content.type === 'd') {
    ...
    //Convert RID values to string if configured
    if (this.server.ridFormat === 's') {
      var keys = Object.keys(value);

      for (var g=0; g<keys.length; g++) {
        var key = keys[g];
        if (value[key] instanceof RID) {
